...
- **CPI_MONTHLY historical-stitch silently failed to update the response's period envelope.** `resp.period` is a plain `dict[str, str | None]` (see `models.DataResponse`), not an object, but the stitch merge did `resp.period.start = first_p` / `resp.period.end = last_p` — attribute assignment on a dict, which raises `AttributeError` every single time the stitch triggered. The enclosing bare `except Exception: pass` swallowed it, so stitched responses shipped with `records`/`row_count` correctly spanning the merged legacy+primary window while `period` silently stayed at the primary-only bounds — a self-contradictory response. Fixed to use dict-item assignment (`resp.period["start"] = ...`). Also narrowed the surrounding `try` to wrap only the legacy upstream fetch (genuine network/API failures stay best-effort and are now logged via `logging.getLogger(__name__).warning(..., exc_info=True)` instead of vanishing); the merge/period-update logic runs unguarded so a future programming error there surfaces instead of hiding behind "stitch is best-effort."
- **`latest()` had no row cap.** Register-shaped curated dataflows (e.g. `C21_G02_SA2`, ~2,400 SA2 areas x 8 measures) could return thousands of rows with zero truncation signal — `DataResponse.truncated_at` was declared in `models.py` but never assigned anywhere in the codebase. Added a `limit` parameter to `latest()` (`Annotated[int, Field(ge=1, le=10000)] = 50`, mirroring ato-mcp's `latest()`), threaded through `_get_data_impl`. When the cap fires, `truncated_at` is set to the pre-cap row count. `get_data()` is intentionally untouched — no default cap — since the gateway and composers rely on full, uncapped series.
- **Same-day follow-up: the row cap above head-sliced (`resp.records[:limit]`), which is the WRONG direction.** `build_response` sorts records ASCENDING by period (oldest first), and `latest()` uses `lastNObservations=1` — when a filter matches multiple series (e.g. several regions), each series carries its OWN last period, and those periods differ across series. Head-slicing the ascending list therefore kept the STALEST matched series and silently dropped the most current ones; `truncated_at` still fired, giving no signal that the *wrong* rows were the ones retained. Same defect class ato-mcp hit and fixed. Added `shaping.truncate_records()` (mirrors `ato_mcp.shaping._truncate_records`): periodic records (non-empty `period`) are tail-sliced (`[-cap:]`) to keep the newest; period-less register/snapshot records (empty `period`, e.g. Census cross-sections) keep head-slicing (`[:cap]`) since source order — not chronology — is what's meaningful there. Mixed lists tail-slice the periodic segment first, then fill any remaining budget from the front of the register segment. `truncated_at` is still the pre-truncation row count.

### Added

- `tests/test_stitch_and_limit.py` — regression coverage for all three fixes: an offline (fully mocked, no live ABS traffic) test asserting the stitched CPI_MONTHLY period spans the full merged window (fails against the old attribute-assignment bug); tests that `latest()` caps at `limit` and sets `truncated_at` to the pre-cap count, defaults to 50, and leaves `truncated_at` at `None` when the result already fits under the cap; and `test_latest_cap_keeps_newest_series_not_oldest`, which drives the PUBLIC `server.latest()` tool with a mixed-period fixture (5 series spanning 2024-01..2026-01, cap=3) and asserts the three MOST RECENT series survive — this test fails against the naive head-slice (kept 2024-01/2024-06/2025-01, the three oldest) and passes with `truncate_records`'s tail-slice. The pre-existing LF fixture (`lf_one_obs.xml`) could not catch this because every series in it happens to share a single `TIME_PERIOD`, so head- vs tail-slicing was indistinguishable — the new fixture forces genuinely different per-series periods.
